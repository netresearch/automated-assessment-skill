...
It loads as `automated-assessment@skills-dir` on the next session. Update with `git -C ~/.claude/skills/automated-assessment pull` and start a new session; remove it by deleting the directory. This route has no `claude plugin update`.
...
npm install --save-dev \
  ...
  github:netresearch/automated-assessment-skill
```
