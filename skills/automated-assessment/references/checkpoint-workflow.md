...
- **`command:`-type checkpoints blocked by the runner's safety sandbox** (`Command rejected: pattern contains command-chaining metacharacter (…)` — covering `;`, `&&`, `||`, backticks and `$()` — or a non-whitelisted base command; the reject reason names the exact rule, and plain pipes are allowed) never ran — report them as INDETERMINATE, not failed. Clear them by rewriting the checkpoint so the allowlist accepts it, or run where the commands are permitted. A command whose executable is absent (`vendor/bin/*` in a project without installed dependencies) is reported as `skip` with `executable not found: <word>`; install the deps to measure it.
