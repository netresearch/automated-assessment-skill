...
# The allowlist lives in lib/command-allowlist.sh so the authoring-time
# validator (validate-checkpoints.sh) applies the exact same rule; a
    ...
    case "$type" in
        file_exists)
            ...
            if [[ "$target" == *"{"*"}"* ]]; then
                ...
                # shellcheck disable=SC2206  # word splitting and globbing are the point here
                local files=($target)
...
while IFS= read -r line; do
    ...
    if [[ "$line" =~ ^llm_reviews:[[:space:]]*$ ]]; then
        ...
        in_mechanical_section=false
        # shellcheck disable=SC2034  # parser state, kept as the twin of in_mechanical_section
        in_llm_section=true
        continue
    fi
